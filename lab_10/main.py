...
from PyQt5.QtWidgets import QRadioButton, QTableWidgetItem, QMessageBox
from PyQt5.QtGui import QImage, QPixmap
from PyQt5.QtCore import Qt


colors = [
    (QtCore.Qt.black, "Черный"),
    (QtCore.Qt.red, "Красный"),
    (QtCore.Qt.green, "Зеленый"),
    (QtCore.Qt.blue, "Синий"),
    (QtCore.Qt.yellow, "Желтый"),
]
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        for i in colors:
            self.color_choose.addItem(i[1])
        self.color_choose.currentIndexChanged.connect(self.set_color)
        ...
        self.infoButton.clicked.connect(print_info)
    ...
    def set_color(self, idx: int):
        self.pen_res.setColor(colors[idx][0])
        self.color_res = colors[idx][0]
...
def print_info(self):
    global wind
    QMessageBox.information(wind, "Информация!", '''Лабораторна работа №10. Чепиго Дарья ИУ7-44Б.
Данная программа реализует алгоритм плавающего горизонта.''')
...
# находим пересечение точки с горизонтом
def horizon(x1, y1, x2, y2, hh, lh, image):
    ...
    # если линия горизонтальная, то рисуем и выходим
    ...
    flag = 0
    if dy > dx: # Меняем местами приращения, чтобы в dy  большее из этих двух приращения и итерация на 1 производилась по dy
        dx, dy = dy, dx
        flag = 1 
