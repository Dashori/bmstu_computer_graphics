...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.info.clicked.connect(print_info)
...
def print_info():
    global wind
    QMessageBox.information(wind, "Информация!", '''Лабораторна работа №8. Чепиго Дарья ИУ7-44Б.
Данная программа реализует алгоритм отсечения отрезка выпуклым отсекателем.''')
...
def end_cutter():
    ...
    if wind.input_cutter:
        if (len(wind.cutter)) == 0:
            QMessageBox.warning(wind, "Ошибка!", "Чтобы замкнуть, введите отсекатель!")
        elif (len(wind.cutter)) < 3:
            QMessageBox.warning(wind, "Ошибка!", "Чтобы замкнуть, отсекатель должен иметь не менее 3-х вершин!")
        else:
